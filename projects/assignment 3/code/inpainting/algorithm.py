...
#
# DO NOT MODIFY THIS FILE ANYWHERE EXCEPT WHERE INDICATED. YOU DO NOT
# NEED TO UNDERSTAND THE CODE IN THIS FILE IN DETAIL. ALL YOU NEED TO
# DO IS IMPLEMENT THE IMAGE-READING AND IMAGE-WRITING ROUTINES FOR PART B.1.4
# OF THE ASSIGNMENT
#
...
#
# Utility function for checking whether the specified image
# has the required size, number of channels, and data type.
# A value of None passed as an argument indicates that the
# particular attribute is unrestricted
#
def isValid(image=None, size=None, channels=None, dtype=None):
    ...
    if channels is None:
        ...
        channelsOK = (channels == 1)
    else:
        channelsOK = (image.shape[2] == channels)
    if (size is None) or (image.shape[0:2] == size):
        sizeOK = True
    else:
        sizeOK = False
    if (dtype is None) or (image.dtype == dtype):
        dtypeOK = True
    else:
        dtypeOK = False
    return channelsOK and sizeOK and dtypeOK
...
class Inpainting:
    ...
    def readImage(self, fileName, key):
        ...
        # since it has two mode, alpha and color
        if key == 'alpha':
            fd = cv.imread(fileName, 0)
        else:
            fd = cv.imread(fileName, 1)


        if fd is not None: # that is fd has been read succussfully
            self._images[key] = fd
            success = True
    ...
    def writeImage(self, fileName, key):
        ...
        if self._images[key] is not None: # if not empty
            write = cv.imwrite(fileName, self._images[key])
...
success, errorMessage = exampleBasedInpainting(self)
        ...
        # ensure the arguments have the correct type/format

        if not isValid(image=self._images['source'], size=None, channels=3, dtype=np.uint8):
            return success, 'source must be a color uint8 image'
        source = self._images['source']
        if not isValid(image=self._images['alpha'], size=source.shape[0:2], channels=1, dtype=np.uint8):
            return success, 'alpha must be a grayscale uint8 image of size equal to source'
        alpha = self._images['alpha']
    ...
    def inpaintRegion(self, imviewer=None, maxIterations=None):
        ...
        if self._fillNewRegion:
            try:
                ...
                boundary = next(self._boundaryIterator)
    ...
    def computeBoundaries(self):
        ...
        boundaries = cv.findContours(unfilled, cv.RETR_LIST, cv.CHAIN_APPROX_NONE)[-2]
    ...
    def fillFrontUpdate(self):
        ...
        newFillFrontCoords = list(filter(addToFillFront, borderCoords))
