...
#
    ...
    def draw_enabled(self):
        for group, enabled in list(self.display_list_groups.items()):
            self.canvas.remove_group(group)
            if enabled:
...
#########################################
    # function to handle a mouse button released pressed event
    def on_touch_up_callback(self, touch):
        ...
        if 'group' in self.ud:
            self.canvas.remove_group(self.ud['group'])
        # delete any xy label we've already drawn
        if 'label' in self.ud:
            # from https://kivy.org/doc/stable/guide/widgets.html
